...
public class KingTest {
    ...
    void testMoves(String in, String count, String value) {
        ...
        King king = new King();
        long actualMoves = king.moves(position);
        int actualCount = king.countMoves(actualMoves);
    ...
    static Stream<Arguments> getData() throws IOException {
        ...
        for (int i = 0; i < 10; i++) {
            String inputFile = String.format("bitboard/king/test.%d.in", i);
            String in = readLines(inputFile).get(0);
            String outputFile = String.format("bitboard/king/test.%d.out", i);
